...
import { readableStreamToString, SnykError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SnykError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SnykError(e, e.response?.data)
                }
                if (innerError instanceof SnykError) {
                    // Got "SnykError" from the above try block
                    throw innerError;
                }
